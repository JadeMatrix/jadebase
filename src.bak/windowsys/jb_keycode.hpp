#ifndef JADEBASE_KEYCODE_HPP
#define JADEBASE_KEYCODE_HPP
 ...
 *   - jadebase performs the same action for Delete/Backspace and Enter/Return,
 *     so these keys are treated as the same.                                   // TODO: Allow discerning difference between these (higher-level events)
 *   - jadebase only accepts Western-type keyboard keys for now.                // TODO: Fix this as part of translations?
 *   - jadebase does not handle the Help key, as there might be a system-
 *     specific action this triggers; help menus ought to be found using in-GUI
 *     tooltips.
 *   - *lock keys are ignored, as they are only reliably useful for text input,
 *     which jadebase allows the platform to handle.
 ...
 *     symbol.                                                                  // TODO: Allow discerning keypad (higher-level events)
